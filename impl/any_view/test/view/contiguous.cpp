...
#define TEST_POINT(x) TEST_CASE(x, "[contiguous_view]")

namespace {

using AnyView =
    std::ranges::any_view<int&, std::ranges::category::contiguous |
...
using AnyViewFull = std::ranges::any_view<
    int&, std::ranges::category::contiguous | std::ranges::category::sized |
...
constexpr void contiguous() {
  std::array v{1, 2, 3, 4, 5};
  V view(std::views::all(v));
  std::same_as<int*> decltype(auto) it1 = view.begin();
  assert(it1 == v[0]);
}
