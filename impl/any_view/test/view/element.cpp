...
constexpr bool test() {
  {
    ...
    static_assert(std::is_same_v<std::ranges::range_reference_t<decltype(view)>, int&>);
    assert(std::ranges::equal(view, v));

    std::ranges::any_view<const int> view2(v);
    static_assert(std::is_same_v<std::ranges::range_reference_t<decltype(view2)>, const int&>);
